...
export default async function EvidenceBoard({ params }: EvidencePageProps) {
  const resolvedParams = await params;
  const game = await prisma.game.findUnique({
    where: { id: resolvedParams.gameId },
    include: { clues: true, characters: true },
  });
  ...
  // A helper to assign stable random rotations and styles to clues based on their index
  const getClueStyle = (index: number) => {
    const rot = [-2, -1, 1, 2, -3, 3][index % 6];
    
    // We alternate clue designs to match the Bento feed aesthetic
      ...
      <main className="pt-24 pb-32 max-w-[1200px] mx-auto px-gutter min-h-screen">
        <header className="mb-12">
          ...
          <p className="font-body-lg text-on-surface-variant max-w-2xl">
            Classified findings from {game.setting}. Cross-reference items to build your timeline. Victim: {game.victim}.
          </p>
        </header>
            ...
            if (style.type === 'polaroid') {
              return (
                <article key={clue.id} className={`transform rotate-${style.rot > 0 ? style.rot : `[-${Math.abs(style.rot)}deg]`} group relative bg-[#f2efe9] p-4 pt-6 pb-12 evidence-shadow transition-transform hover:rotate-0 hover:scale-105 duration-300`} style={{transform: `rotate(${style.rot}deg)`}}>
                  ...
                  <p className="text-xs text-black/80 mt-2 font-body-md text-center">{clue.description}</p>
                </article>
              );
            }
            ...
            return (
              <article key={clue.id} className={`group relative glass-card p-4 evidence-shadow min-h-[350px]`} style={{transform: `rotate(${style.rot}deg)`}}>
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs font-body-md text-on-surface-variant">{clue.description}</p>
                </div>
        ...
        {/* Suspect Quick Links (Bonus feature to get to characters) */}
        <div className="mt-16">
